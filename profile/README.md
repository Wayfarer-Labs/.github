# Overworld

## The Local World Modeling Company

**Simulate Anything on Anything** - Overworld builds AI that dreams playable worlds — 60 FPS, sub-20ms latency, actually fun. We're closing the gap between what people imagine and what they can experience. The ultimate sandbox, and it's real.

[Join Our Discord](https://discord.gg/overworld) | [Visit Our Website](https://over.world/) | [Try our model](https://overworld.stream)

## Recent Publications

- [Why Human Evaluation is the Missing Piece in World Model Development](https://over.world/blog/why-human-evaluation-is-the-missing-piece-in-world-model-development)
- [Optimizing Diffusion Decoders with Depth Pruning](https://over.world/blog/diffdec-ode-regression)
- [Generation vs. Reconstruction: Striking A Balance](https://over.world/blog/gen-vs-rec)
- [Fast Audio Video World Models](https://over.world/blog/fast-audio-video-world-models-part-2)
- [Training Diffusable Autoencoders](https://over.world/blog/training-diffusable-autoencoders)

## Who Should Get Involved?

- **Researchers** - Collaborate on cutting-edge world model research
- **Developers** - Build applications with our models or contribute code


---

*Built by the Overworld team in Providence, Rhode Island*
